...
# ucitaj podatke
data = pd.read_csv("LV6 Klasifikacija primjenom algoritma K najbližih susjeda i primjenom strojeva s potpornim vektorima (1)\Social_Network_Ads.csv")
print(data.info())
...
def KNN(neighbour: int):
    ...
    plt.ylabel('x_2')
    plt.legend(loc='upper left')
    plt.title(f"Tocnost za {neighbour}: " + "{:0.3f}".format((accuracy_score(y_train, y_train_p_KNN))))
    plt.tight_layout()
    plt.show()
